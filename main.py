...
def buscar_nagumo(term):
    ...
    while True:
        ...
        try:
            ...
            # O Nagumo retorna HTML neste endpoint, mas os produtos ficam embutidos 
            # no atributo products="[{...}]" da tag customizada <search-card-grid>
            match = re.search(r'<search-card-grid[^>]*products="([^"]+)"', r.text)
            if not match:
                break
                
            products_json_str = html.unescape(match.group(1))
            products_page = json.loads(products_json_str)
            ...
            # Condição de parada (menos itens retornado do que o page size estipulado, ou limite máximo de segurança)
            if len(products_page) < sz or start >= 100:
                break
...
if termo:
    ...
    with st.spinner("🔍 Buscando no Nagumo..."):
        ...
        for p in raw_nagumo:
            sku = p.get('id')
            # Check-in de disponibilidade que agora é um campo próprio
            if sku and sku not in vistos_nagumo and p.get('available', True):
                ...
                if all(k in remover_acentos(f"{nome} {desc}") for k in palavras_chave):
                    ...
                    if not preco_info: continue
                        
                    sales = preco_info.get('sales', {}) or {}
                    preco_normal = sales.get('value', 0)
                    preco_final = preco_normal
                    
                    # Checando descontos/promoções (que agora vêm no array 'flagtypes')
                    flagtypes = p.get('flagtypes') or []
                    ...
                    url_final = p.get('productShowFullUrl') or f"https://www.nagumo.com.br/categoria/departamentos/p/{slugify(nome)}-{sku}.html"
                    p['url_final'] = url_final
                    
                    # Unidade e Label baseado se é um item pesável (kg)
                    if p.get('weighable'):
                        label = f"R$ {preco_final:.2f}/kg"
                    else:
                        label = calcular_preco_unitario_nagumo(preco_final, desc, nome, None)
                    ...
                    # Normalização para a interface streamlit baseada nas chaves antigas
                    p['name'] = nome
                    p['description'] = desc
                    p['stock'] = p.get('ATSInCurrentStore', p.get('ATSInGenerealStock', 0))
                    
                    try:
                        p['photosUrl'] = [p['images']['large'][0]['src']['disUrl']]
                    except:
                        p['photosUrl'] = []
