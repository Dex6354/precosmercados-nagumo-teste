...
def texto_corresponde_busca(texto, palavras_chave):
    """
    Correspondência mais flexível:
    - aceita singular/plural
    - aceita início da palavra
    - evita perder itens válidos da API
    """
    ...
    for palavra in palavras_chave:
        ...
        for v in variantes:
            if v and (
                v in texto or
                any(p.startswith(v) for p in texto.split())
            ):
                achou = True
                break
    ...
    # aceita se encontrou ao menos 1 palavra
    return encontrados >= max(1, len(palavras_chave) // 2)
...
def extrair_info_papel_toalha(nome, descricao):
    ...
    for texto in [texto_nome, texto_completo]:
        ...
        match = re.search(
            r'(\d+)\s*(un|unidades?|rolos?)\s*.*?(\d+)\s*(folhas|toalhas)',
            texto
        )
...
def calcular_preco_unitario_nagumo(preco_valor, descricao, nome, unidade_api=None):
    ...
    if contem_papel_toalha(texto_completo):
        ...
        rolos, folhas, total_folhas, txt = extrair_info_papel_toalha(
            nome,
            descricao
        )
    ...
    if "papel higi" in texto_completo:
        ...
        m_rolos = re.search(
            r"(leve\s*0*|lv?\s*0*|lv?|l\s*0*|c/\s*0*)(\d+)",
            texto_completo
        )

        if not m_rolos:
            m_rolos = re.search(
                r"(\d+)\s*(rolos?|un|unidades?)",
                texto_completo
            )
        ...
        if m_rolos and m_metros:
            try:
                rolos = int(
                    m_rolos.group(2)
                    if m_rolos.lastindex > 1
                    else m_rolos.group(1)
                )

                metros = float(
                    m_metros.group(1).replace(',', '.')
                )
    ...
    for fonte in fontes:
        ...
        if match_g:
            valor = float(match_g.group(1).replace(',', '.'))

            if valor > 0:
                return f"R$ {preco_valor / (valor / 1000):.2f}/kg"

        match_kg = re.search(r"(\d+[.,]?\d*)\s*(kg|quilo)", fonte)

        if match_kg:
            valor = float(match_kg.group(1).replace(',', '.'))

            if valor > 0:
                return f"R$ {preco_valor / valor:.2f}/kg"

        match_ml = re.search(r"(\d+[.,]?\d*)\s*(ml|mililitros?)", fonte)

        if match_ml:
            valor = float(match_ml.group(1).replace(',', '.'))

            if valor > 0:
                return f"R$ {preco_valor / (valor / 1000):.2f}/L"

        match_l = re.search(r"(\d+[.,]?\d*)\s*(l|litros?)", fonte)

        if match_l:
            valor = float(match_l.group(1).replace(',', '.'))

            if valor > 0:
                return f"R$ {preco_valor / valor:.2f}/L"

        match_un = re.search(r"(\d+[.,]?\d*)\s*(un|unidades?)", fonte)

        if match_un:
            valor = float(match_un.group(1).replace(',', '.'))

            if valor > 0:
                return f"R$ {preco_valor / valor:.2f}/un"
...
if termo:
    ...
    termos_busca = gerar_formas_variantes(
    ...
    with st.spinner("🔍 Buscando no Nagumo..."):
        ...
        for p in raw_nagumo:
            ...
            if sku and sku not in vistos_nagumo:
                ...
                texto_busca = f"{nome} {desc} {slugify(nome)}"

                if texto_corresponde_busca(
                    texto_busca,
                    palavras_chave
                ):
                    ...
                    p['url_final'] = (
                        f"https://www.nagumo.com.br/"
                        f"categoria/departamentos/p/"
        ...
        nagumo_final = sorted(
            nagumo_final,
            key=lambda x: x['sort_val'] or 999
        )
    ...
    with col_center:
        ...
        <h5 style="display: flex; align-items: center; justify-content: center;">
            <img src="{LOGO_NAGUMO_URL}" width="100">
        </h5>
        """, unsafe_allow_html=True)

        st.markdown(
            f"<p align='center'><small>🔎 {len(nagumo_final)} produto(s) encontrado(s).</small></p>",
            unsafe_allow_html=True
        )
