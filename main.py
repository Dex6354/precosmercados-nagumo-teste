...
def calcular_preco_unitario_nagumo(preco_valor, nome, medida_venda):
    ...
    # Tenta extrair peso/volume do nome do produto
...
def buscar_nagumo(term):
    all_products = []
    # Busca start 0 e 20 para cobrir mais resultados
    for start in [0, 20]:
        ...
        for p in raw_nagumo:
            ...
            if pid and pid not in vistos:
                ...
                if all(k in remover_acentos(nome) for k in palavras_chave):
                    ...
                    preco_venda = price_obj.get('value', 0)
                    try: preco_venda = float(preco_venda)
                    except: preco_venda = 0.0

                    # Preço "Meu Nagumo" (se houver flag)
                    ...
                    # Montagem do objeto para exibição
                    p['calc_label'] = label
            ...
            st.markdown(f"""
                <div class='product-container'>
                    ...
                    <div class='product-info'>
                        ...
                        <div style="color: gray; font-size: 0.7rem;">Marca: {p.get('brand', 'Hortifruti')}</div>
                    </div>
                </div>
