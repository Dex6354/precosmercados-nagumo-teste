...
def gerar_formas_variantes(termo):
    ...
    # Variantes extras para hortifruti
    if termo in ["cenoura", "banana", "maca", "laranja", "tomate"]:
        variantes.add(termo + "s")
        variantes.add(termo)
...
def calcular_preco_unitario_nagumo(preco_valor, descricao, nome, unidade_api=None):
    # ... (mantido igual ao seu código original - não alterei essa parte)
...
st.markdown("""
    <style>
        ...
        .product-container { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 0.5rem; flex-wrap: wrap; }
...
if termo:
    ...
    with st.spinner("🔍 Buscando no Nagumo..."):
        ...
        for p in raw_nagumo:
            ...
            # Filtro mais tolerante (melhor para cenoura e hortifruti)
    ...
    with col_center:
        ...
        if not nagumo_final:
            st.warning("Nenhum produto encontrado. Tente outro termo ou verifique o nome.")
