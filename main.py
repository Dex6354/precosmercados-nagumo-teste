...
# --- REQUISIÇÃO NAGUMO (VIA SCRAPING REGEX) ---
...
if termo:
        ...
        for p in raw_nagumo:
            ...
            if sku and sku not in vistos_nagumo:
                ...
                if all(k in remover_acentos(f"{nome} {desc}") for k in palavras_chave):
                    # Mapeamento de preços do JSON injetado
    ...
    # Centralizando a coluna única (Layout original)
    ...
    with col_center:
        ...
        for p in nagumo_final:
            # No novo JSON as fotos podem vir em 'images' ou 'photosUrl'
