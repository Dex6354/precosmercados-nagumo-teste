...
def buscar_nagumo(term):
    ...
    for _ in range(5): # Limite de 5 páginas
        ...
        try:
            ...
            # O Nagumo agora coloca os dados em data-props
            match = re.search(r'data-props="([^"]+)"', r.text)
            
            if match:
                content = html.unescape(match.group(1))
                data = json.loads(content)
                # No UpdateGrid, os produtos costumam vir direto ou em uma chave 'products'
    ...
    with col_center:
        ...
        for p in nagumo_final:
            ...
            if isinstance(items, list) and len(items) > 0:
                ...
                img = raw['images'][0].get('imageUrl', img) if isinstance(raw['images'][0], dict) else raw['images'][0]
