...
def calcular_preco_unitario_nagumo(preco_valor, descricao, nome, unidade_api=None):
    ...
    # Tenta extrair unidade do texto (Kg, L, Un)
    ...
    # Fallback para unidade da API (comum em itens de hortifruti como a Cenoura)
    if unidade_api:
        u = str(unidade_api).lower()
        if 'kg' in u: return f"R$ {preco_valor:.2f}/kg"
        elif 'g' in u: return f"R$ {preco_valor * 1000:.2f}/kg"
        elif 'l' in u: return f"R$ {preco_valor:.2f}/L"
        elif 'ml' in u: return f"R$ {preco_valor * 1000:.2f}/L"
        elif 'un' in u or 'unit' in u: return f"R$ {preco_valor:.2f}/un"
    
    return f"Preço: R$ {preco_valor:.2f}"
...
if termo:
    ...
    with st.spinner("🔍 Buscando no Nagumo..."):
        ...
        for p in raw_nagumo:
            ...
            if sku and sku not in vistos_nagumo:
                ...
                # Validação de palavra-chave para evitar lixo
                if all(k in remover_acentos(f"{nome} {desc}") for k in palavras_chave):
                    ...
                    # Se o preço final for 0, ignorar
                    if preco_final <= 0: continue

                    p['url_final'] = f"https://www.nagumo.com.br/categoria/departamentos/p/{slugify(nome)}-{sku}.html"
                    
                    # Lógica do main2: usar o campo 'unit' da API como prioridade para itens sem peso na descrição
    ...
    with col_center:
        ...
        for p in nagumo_final:
            ...
            if preco_final < preco_normal and preco_final > 0:
