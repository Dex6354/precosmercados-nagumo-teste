...
termo = st.text_input("🔎 Pesquisar:", "Cenoura").strip()

if termo:
    palavras_chave = remover_acentos(termo).split()
    with st.spinner("Buscando na API..."):
        ...
        for p in resultados:
            ...
            if sku and sku not in vistos:
                ...
                # --- LÓGICA DE PREÇO ROBUSTA (EXTRAÍDA DO COMPORTAMENTO DO SITE) ---
                promo = p.get('promotion') or {}
                cond = promo.get('conditions') or []
                preco_base = p.get('price', 0)
                
                # Prioridade 1: Preço da Promoção Ativa
                # Prioridade 2: Preço Base da API
                preco_final = cond[0].get('price') if (promo.get('isActive') and cond) else preco_base
                
                # Se ainda for 0, tenta pegar o preço base ignorando a flag de ativação
                if preco_final <= 0 and cond:
                    preco_final = cond[0].get('price')
    ...
    with col_center:
        if not final_list:
            st.warning("Nenhum item com preço válido encontrado. Verifique se o item está disponível para a loja 22.")
    ...
    with st.expander("📝 Log Técnico da Busca (F12 local)"):
        st.write(f"Total de itens retornados pela API: {len(resultados)}")
        for r in resultados:
            st.text(f"SKU: {r.get('sku')} | Nome: {r.get('name')} | Preço API: {r.get('price')} | Promo: {bool(r.get('promotion'))}")
